...
function LeadScraper({ state, setState, onAdd, pipelineNames, pipeline }) {
  ...
  // ── THE FIX: filter out dismissed AND already-pipelined leads ──
  const visible = prospects.filter(p => !dismissed.has(p.name) && !pipelineNames.has(p.name));
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {!loading && visible.length === 0 && prospects.length > 0 && (
        <Card><p style={{ fontFamily: MONO, fontSize: 11, color: "rgba(255,255,255,0.13)", margin: 0 }}>All results dismissed or already in pipeline — search a new niche or clear results.</p></Card>
      )}
